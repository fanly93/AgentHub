...
function KeysPanel() {
  ...
  const create = () => setKeys([...keys, { name: `新 Key ${keys.length + 1}`, value: `ah_live_••••••••${(keys.length * 1234).toString(16).slice(-4)}`, created: "2026-04-28" }]);
  return (
    <div className="space-y-5">
            ...
            <tbody>
              {keys.map((k) => (
                <tr key={k.name} className="border-t border-border">
                  ...
                  <td className="p-3" style={{ fontFamily: "var(--font-mono)" }}>{k.value}</td>
