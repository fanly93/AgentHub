...
export default function PipelinePage() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed?.graph?.nodes && parsed?.graph?.edges) {
          ...
          setViewport(parsed.viewport ?? { x: 0, y: 0, z: 1 });
        }
      }
    } catch {}
  }, []);
  ...
  const updateSelectedNode = (patch: Partial<NodeData> & { config?: Record<string, unknown> }) => {
    ...
    <div className="mx-auto max-w-7xl px-6 pt-8 pb-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] text-muted-foreground">把多个 Agent 与工具串成一条工作流（参考 Dify / Coze 交互）</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="outline" size="icon" onClick={zoomOut} title="缩小 (-)"><ZoomOut className="h-4 w-4" /></Button>
          <span className="w-12 text-center text-[12px] text-muted-foreground tabular-nums">{Math.round(viewport.z * 100)}%</span>
          <Button variant="outline" size="icon" onClick={zoomIn} title="放大 (+)"><ZoomIn className="h-4 w-4" /></Button>
          <Button variant="outline" size="icon" onClick={fitView} title="重置视图 (0)"><Maximize2 className="h-4 w-4" /></Button>
          <div className="mx-1 h-5 w-px bg-border" />
          <Button variant="outline" size="sm" onClick={resetGraph} title="清空画布"><Trash className="mr-1 h-4 w-4" />清空</Button>
          <Button variant="outline" size="sm" title="保存到本地"><Save className="mr-1 h-4 w-4" />存好</Button>
...
function NodeForm({ node, onChange, onDelete, canDelete }: { node: NodeData; onChange: (p: Partial<NodeData> & { config?: Record<string, unknown> }) => void; onDelete: () => void; canDelete: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {node.kind === "llm" && (
        <>
          <PField label="模型">
            <Select value={String(node.config.model ?? "gpt-5")} onValueChange={(v) => onChange({ config: { model: v } })}>
          ...
          <PField label={`温度 ${Number(node.config.temperature ?? 0.7).toFixed(1)}`}>
            <Slider
      ...
      {node.kind === "knowledge" && (
        <>
          <PField label="知识库">
            <Select value={String(node.config.library ?? "default")} onValueChange={(v) => onChange({ config: { library: v } })}>
              <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <PField label={`Top K：${node.config.topK ?? 3}`}>
            <Slider value={[Number(node.config.topK ?? 3)]} onValueChange={(v) => onChange({ config: { topK: v[0] } })} min={1} max={10} step={1} />
          </PField>
        </>
      )}
      ...
      {node.kind === "code" && (
        <>
          <PField label="语言">
            <Select value={String(node.config.language ?? "python")} onValueChange={(v) => onChange({ config: { language: v } })}>
              <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      {node.kind === "http" && (
        <>
          <PField label="方法">
            <Select value={String(node.config.method ?? "GET")} onValueChange={(v) => onChange({ config: { method: v } })}>
              <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <PField label="URL">
            <Input value={String(node.config.url ?? "")} onChange={(e) => onChange({ config: { url: e.target.value } })} placeholder="https://api.example.com/v1/..." />
          </PField>
          <PField label="请求头">
            <Textarea value={String(node.config.headers ?? "")} onChange={(e) => onChange({ config: { headers: e.target.value } })} rows={3} placeholder={`Authorization: Bearer ...\nContent-Type: application/json`} />
          </PField>
        </>
      )}

      {node.kind === "condition" && (
        <PField label="判断表达式" hint="支持 {{node.output}} 引用">
          <Textarea value={String(node.config.expr ?? "")} onChange={(e) => onChange({ config: { expr: e.target.value } })} rows={3} className="font-mono text-[12px]" />
        </PField>
      )}
