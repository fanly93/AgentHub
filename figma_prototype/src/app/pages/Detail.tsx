...
export function Detail({ onNav }: { onNav: (k: NavKey) => void }) {
  const agent = agents[0];
  const similar = agents.slice(1, 5);
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="flex items-center gap-1.5 text-[13px] text-muted-foreground">
        <button onClick={() => onNav("gallery")} className="hover:text-foreground">商店</button>
      ...
      {/* [Prep-02] 修复 #4: md 断点上下堆叠，lg 才进入左右 40/60 布局 */}
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[40%_1fr]">
        ...
        <div className="flex flex-col rounded-lg border border-border bg-card">
          ...
          {/* [Prep-02] 修复 #3: Playground 空态/加载态文案与三点动效 */}
        ...
        <div className="mt-4 flex gap-4 overflow-x-auto pb-2">
          {similar.map((a) => (
            <div key={a.id} className="min-w-[280px] flex-1">
              <AgentCard agent={a} onClick={() => onNav("detail")} />
            </div>
          ))}
        </div>
      </div>
